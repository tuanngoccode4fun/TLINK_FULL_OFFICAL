...
{
  public  class LoadPlanningItemsA01
    {
        public List<DataHeaderPlanning> GetDataHeaderPlanningA01GiaDung(string dept, DateTime from, DateTime to, out Dictionary<string, List<dataContent>> ListPlanShipments, out List<SemiFinishedGoods> semiFinishedGoods) 
        {
           ...
           var     orderVariables = loadData.LoadOrderInformationbyDatebyDept(from, to, dept);
                ...
                var GetlistFinishedGoods = ListPlanning.Values.AsEnumerable().Select(d => d.SemiFinishedGoods).ToList().SelectMany(x => x).ToList(); ;


                listSemiFinishedGoods = GetlistFinishedGoods;
                ...
                foreach (var item in ListPlanning)
                {
                   var datacontents = new List<dataContent>();
                    ...
                    for (int i = 0; i < item.Value.SemiFinishedGoods.Count; i++)
                    {
                        ...
                        data.Semi_FGs_Needed_Qty += item.Value.SemiFinishedGoods[i].QtyNeed.ToString("N0") + Environment.NewLine;
                        data.StockSemiFinished += item.Value.SemiFinishedGoods[i].QtyInWarehouse.ToString("N0") + Environment.NewLine;
    ...
    .ToList();
                    ...
                    foreach (var shipment in item.Value.shipmentPlans)
                    {
                        ...
                        dataContent.OrderQty = shipment.NeedQty;
                        dataContent.Deliveried = shipment.DeliveryPlanQty;
                        ShipmentQtySum += dataContent.OrderQty;
                        dataContent.ShortageQty = dataContent.OrderQty- shipment.DeliveryPlanQty;
                        ...
                        else dataContent.TargetQtyDay = (dataContent.ShortageQty < 0) ? 0 : Math.Ceiling((dataContent.ShortageQty / dataContent.RemainDay));
            ...
            semiFinishedGoods = listSemiFinishedGoods;
            return headerPlanningA01s;
           
        }

        public List<DataHeaderPlanning> GetDataHeaderPlanningA01PTC(string dept, DateTime from, DateTime to, out Dictionary<string, List<dataContent>> ListPlanShipments)
        {
            List<DataHeaderPlanning> headerPlanningA01s = new List<DataHeaderPlanning>();
            try
            {
                ...
                foreach (var item in ListPlanning)
                {
                    ...
                    for (int i = 0; i < item.Value.SemiFinishedGoods.Count; i++)
                    {
                        ...
                        data.SemiFinishedGoods += item.Value.SemiFinishedGoods[i].Item + Environment.NewLine;
                        data.StockInWarehouse += item.Value.SemiFinishedGoods[i].QtyWarehouse.ToString("N0") + Environment.NewLine;
                        data.MQCStock += item.Value.SemiFinishedGoods[i].QTyAtMQC.ToString("N0") + Environment.NewLine;
                        data.PQCStock += item.Value.SemiFinishedGoods[i].QTyAtPQC.ToString("N0") + Environment.NewLine;
                        data.PendingWarehouse += item.Value.SemiFinishedGoods[i].QtyPendingWarehouse.ToString("N0") + Environment.NewLine;
                        if (item.Value.SemiFinishedGoods[i].accessories != null)
    ...
    .ToList();
        ...
        public List<DataHeaderPlanning> GetDataHeaderPlanningA01JM(string dept, DateTime from, DateTime to, out Dictionary<string, List<dataContent>> ListPlanShipments)
        {
            List<DataHeaderPlanning> headerPlanningA01s = new List<DataHeaderPlanning>();
            try
            {
                ...
                foreach (var item in ListPlanning)
                {
                    ...
                    for (int i = 0; i < item.Value.SemiFinishedGoods.Count; i++)
                    {
                        ...
                        data.SemiFinishedGoods += item.Value.SemiFinishedGoods[i].Item + Environment.NewLine;
                        data.Semi_FGs_Needed_Qty += item.Value.SemiFinishedGoods[i].QtyNeed.ToString("No") + Environment.NewLine;
                        data.StockInWarehouse += item.Value.SemiFinishedGoods[i].QtyWarehouse.ToString("N0") + Environment.NewLine;
                        data.MQCStock += item.Value.SemiFinishedGoods[i].QTyAtMQC.ToString("N0") + Environment.NewLine;
                        data.PQCStock += item.Value.SemiFinishedGoods[i].QTyAtPQC.ToString("N0") + Environment.NewLine;
                        data.PendingWarehouse += item.Value.SemiFinishedGoods[i].QtyPendingWarehouse.ToString("N0") + Environment.NewLine;
